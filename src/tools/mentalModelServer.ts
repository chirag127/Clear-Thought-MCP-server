...
export class MentalModelServer {
  private validateModelData(input: unknown): MentalModelData {
    ...
    return {
      ...
      reasoning: typeof data.reasoning === 'string' ? data.reasoning as string : '',
      conclusion: typeof data.conclusion === 'string' ? data.conclusion as string : ''
    };
  }
  ...
  private formatModelOutput(data: MentalModelData): string {
    ...
    let output = `\n${chalk.bold.blue('Mental Model:')} ${chalk.bold(modelName)}\n`;
